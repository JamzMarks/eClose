...
import { ONBOARDING_SLIDES } from "@/onboarding/onboarding-slides";
...
export default function OnboardingRoute() {
  ...
  const { hydrated, introPending, completeIntro, skipIntro } = useOnboarding();
  ...
  const onScroll = useCallback((e: NativeSyntheticEvent<NativeScrollEvent>) => {
    ...
    setPage(Math.max(0, Math.min(ONBOARDING_SLIDES.length - 1, i)));
  }, []);

  const goNext = useCallback(() => {
    if (page >= ONBOARDING_SLIDES.length - 1) return;
    const next = page + 1;
